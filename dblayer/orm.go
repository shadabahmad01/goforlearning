...
import (
	...
	"github.com/shadab/gomod/models"
...
func hashPassword(s *string) error {
	...
	hashdeBytes, err := bcrypt.GenerateFromPassword(sBytes, bcrypt.DefaultCost)
	if err != nil {
		return err
	}
	//update password string with the hashed version
	*s = string(hashdeBytes[:])
	return nil
}
...
func (db *DBORM) SignOutUserById(id int) error {
	...
	//update the customer row to reflect the fact that the customer is not loogged in
...
func (db *DBORM) GetCustomerOrdersByID(id int) (orders []models.Order, err error) {
	return orders, db.Table("orders").Select("*").Joins("join customers on customers on customer.id = customer_id").Joins("join products on products.id = product_id").Where("customer_id=?").Scan(&orders).Error
}
